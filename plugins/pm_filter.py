...
async def pm_next_page(bot, query):
    ...
    btn.insert(0, [InlineKeyboardButton("🔗 ʜᴏᴡ ᴛᴏ ᴅᴏᴡɴʟᴏᴀᴅ 🔗", "https://t.me/saurabh_1710")])
...
async def pm_AutoFilter(client, msg, pmspoll=False):    
    if not pmspoll:
        ...
        if len(message.sticker) < 100:
            search = message.sticker
            m=await message.reply_sticker("CAACAgUAAxkBAAINE2VXHXJfHRyi4Sn--Wd6Pj4R54gLAAKcAAPIlGQUc48AAfPaxYX8HgQ")
    ...
    btn.insert(0, [InlineKeyboardButton("🔗 ʜᴏᴡ ᴛᴏ ᴅᴏᴡɴʟᴏᴀᴅ 🔗", url="https://t.me/saurabh_1710")])
    if offset != "":
        key = f"{message.id}"
